...
class Net(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.s_dim = s_dim
        self.a_dim = a_dim
        self.map_len = (s_dim[0]-1)*s_dim[1]*s_dim[2] # the length of the vector stretched by the map (the first 3 channels)  policy actor
        ...
        self.v1 = nn.Linear(self.map_len, 128)  # critic
    ...
    def get_action_and_value(self, input, action=None):
        if len(input.shape)==3:
            input = input.reshape(1,4,30,30) # for single input, batch = 1
        p = input[:,3,0,0:2].flip(0) #position
        x = input[:,0:3,:,:].flatten(-3) #map, stretched
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.get_value(input)
    ...
    def loss_func(self, s, a, v_t):
        ...
        td = v_t - values
        c_loss = td.pow(2)  # how better than last version
        probs = F.softmax(logits, dim=1)
        m = self.distribution(probs)
        exp_v = m.log_prob(a) * td.detach().squeeze()  # log * advantage
        a_loss = -exp_v
